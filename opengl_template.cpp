...
void draw () {
    ...
    */
    ...
    // Если выбран режим двойной буферизации - Gl_DOUBLE:
    // glutSwapBuffers();
...
void light_init(){
    /** Свет применяется исключительно при задании необходимой геометрии: нормалей!!!!!
     ...
     *
     * Точечный источник света:     расположен в конкретной точке пространства и светит равномерно во всех направлениях.
     *                              Для него можно задать эффект затухания света с расстоянием.
     *
     * Прожектор:                   является частным случаем точечного источника,
     *                              но свет от него распространяется только внутри ограничивающего конуса,
     *                              а не по всем направлениям.
     */

    // По умолчанию - свет отключён
    glEnable(GL_LIGHTING);

    glLightModelf(GL_LIGHT_MODEL_TWO_SIDE, GL_TRUE); // делаем так, чтобы освещались обе стороны полигона
    /** glLightModel[i f]v(GLenum pname, const GLtype *params)
     *  glLightModel[i f](GLenum pname, GLenum param)
     *
     * GL_LIGHT_MODEL_AMBIENT       параметр params должен содержать четыре целых или вещественных числа,
     *                              которые определяют цвет фонового освещения
     *                              даже в случае отсутствия определенных источников света.
     *
     * GL_LIGHT_MODEL_TWO_SIDE      параметр param должен быть булевским и управляет режимом расчета освещенности
     *                              как для лицевых, так и для обратных граней.
     *                              Если он равен FALSE, то освещенность рассчитывается только для лицевых граней.
     *                              Если же он равен TRUE, расчет проводится и для обратных граней.
     *                              Значение по умолчанию: FALSE.
     *
     * GL_LIGHT_MODEL_LOCAL_VIEWER  параметр param должен быть булевским и задает положение наблюдателя.
     *                              Если он равен FALSE, то направление обзора считается параллельным оси -z,
     *                              вне зависимости от положения в видовыx координатах.
     *                              Если же он равен TRUE, то наблюдатель находится в начале видовой системы координат.
     *                              Это может улучшить качество освещения, но усложняет его расчет.
     *                              Значение по умолчанию: FALSE.
     */
    ...
    glLightfv(GL_LIGHT0, GL_DIFFUSE, light0_diffuse); // устанавливаем источнику света light0 диффузный свет
    glLightfv(GL_LIGHT0, GL_POSITION, light0_direction); // устанавливаем направление источника света
}
